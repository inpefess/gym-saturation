...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    cnf(2, lemma, mult(e,X0) = X0, inference(input, [], [])).
    cnf(9, lemma, mult(inv(X0),mult(X0,X1)) = X1, inference(forward_demodulation, [], [6, 2])).
    cnf(4, lemma, a = mult(a,a), inference(input, [], [])).
    cnf(18, lemma, a = mult(inv(a),a), inference(superposition, [], [9, 4])).
    cnf(3, lemma, e = mult(inv(X0),X0), inference(input, [], [])).
    cnf(22, lemma, e = a, inference(forward_demodulation, [], [18, 3])).
    cnf(5, lemma, e != a, inference(input, [], [])).
    cnf(23, lemma, $false, inference(subsumption_resolution, [], [22, 5])).
...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    Loop <_UnixSelectorEventLoop running=False closed=True debug=False> that handles pid 62385 is closed
